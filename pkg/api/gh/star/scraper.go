...
func NewScraper(ctx context.Context, gh *github.Client, opts ...Option) *scraper {
	copts := Options{}
...
func (g *scraper) Discover() (api.API, error) {
	a := NewRepoAPI(g.src)

	repo := NewResource("repo", "repos", version, resKind, true, api.Options{Metadata: metadata.New()})
	...
	lang := NewResource("lang", "langs", version, resKind, false, api.Options{Metadata: metadata.New()})
	if err := a.Add(lang, api.AddOptions{}); err != nil {
		return nil, err
	}

	topic := NewResource("topic", "topics", version, resKind, false, api.Options{Metadata: metadata.New()})
...
func (g *scraper) mapObjects(top *Top, linkTo uuid.UID, res api.Resource, rel string, names []string) ([]*Object, error) {
...
func (g *scraper) mapRepos(reposChan <-chan []*github.StarredRepository, top *Top, res resources) error {
...
func (g *scraper) Map(a api.API) (api.Top, error) {
	top := NewTop(a)
